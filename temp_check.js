...
// ============================================================
    ...
    function savePlant(id) {
      ...
      if (!saved.includes(id)) { saved.push(id); localStorage.setItem('plantglow_saved', JSON.stringify(saved)); }
    }

    function unsavePlant(id) {
      var saved = getSavedPlants().filter(function(pid) { return pid !== id; });
      localStorage.setItem('plantglow_saved', JSON.stringify(saved));
    }

    function handleToggleSave(plantId) {
      if (!isLoggedIn()) { openLoginRequiredModal(); return; }
      var saved = getSavedPlants();
      if (saved.includes(plantId)) { unsavePlant(plantId); showToast('Removed from favorites', 'info'); }
      else { savePlant(plantId); showToast('Saved to favorites!', 'success'); }
      renderPlants();
    }
    ...
    function renderPlantCard(id, plant) {
      ...
      var heartFill = isSaved ? '#ef4444' : '#ffffff';
      var heartStroke = isSaved ? '#ef4444' : '#d1d5db';
      return '<div class="plant-card bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer relative" onclick="handlePlantClick(\'' + id + '\')">' +
        ...
        '</div></div>' +
        '<button onclick="event.stopPropagation(); handleToggleSave(\'' + id + '\')" class="absolute top-2 right-2 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition">' +
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="' + heartFill + '" stroke="' + heartStroke + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>' +
        '</button></div>';
    }
