...
const before = html.substring(0, mainScriptStart);
const after = html.substring(mainScriptEnd);

// Clean JavaScript with no Unicode chars in comments
const cleanJS = `
    ...
    function updateNav() {
      ...
      document.getElementById('nav-logged-out').classList.toggle('hidden', loggedIn);
      document.getElementById('nav-logged-in').classList.toggle('hidden', !loggedIn);
      document.getElementById('nav-logged-in').classList.toggle('flex', loggedIn);

      document.getElementById('mobile-nav-logged-out').classList.toggle('hidden', loggedIn);
      document.getElementById('mobile-nav-logged-in').classList.toggle('hidden', !loggedIn);
    ...
    function renderPlants() {
      ...
      const browseText = document.getElementById('browse-all-text');
      const loggedIn = isLoggedIn();
      const plantIds = Object.keys(plantData);

      const showCount = (loggedIn || showingAll) ? plantIds.length : VISIBLE_COUNT;
      const plantsToShow = plantIds.slice(0, showCount);

      grid.innerHTML = plantsToShow.map(id => renderPlantCard(id, plantData[id])).join('');
    ...
    function showToast(message, type) {
      ...
      toast.className = 'fixed top-20 left-1/2 transform -translate-x-1/2 z-[200] px-6 py-3 rounded-xl shadow-lg text-white font-medium text-sm flex items-center gap-2 transition-all duration-300 ' + (type === 'success' ? 'bg-forest-600' : type === 'info' ? 'bg-blue-500' : 'bg-red-500');
      toast.innerHTML = '<span>' + (type === 'success' ? 'OK' : type === 'info' ? 'INFO' : 'ERROR') + '</span><span>' + message + '</span>';
    ...
    function openPlantModal(plantId) {
      ...
      const tipsEl = document.getElementById('plant-modal-tips');
      tipsEl.innerHTML = '';
      ...
      const problemsEl = document.getElementById('plant-modal-problems');
      ...
      const premiumEl = document.getElementById('plant-modal-premium');
      if (subscribed) {
...
console.log('Done! Rewritten with clean JS.');
console.log('Before length:', before.length, 'JS length:', cleanJS.length, 'After length:', after.length);