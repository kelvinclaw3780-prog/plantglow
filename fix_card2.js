...
// Find renderPlantCard function bounds
const funcStart = content.indexOf('function renderPlantCard');
const afterFunc = content.substring(funcStart);
const funcEnd = funcStart + afterFunc.indexOf('function renderPlants');

// Build new function
const newRenderPlantCard = `function renderPlantCard(id, plant) {
...
// Replace just the renderPlantCard function (up to function renderPlants)
const before = content.substring(0, funcStart);
const after = content.substring(funcEnd);
const newContent = before + newRenderPlantCard + after;

fs.writeFileSync(path, newContent, 'utf8');
console.log('Done! Replacement length:', newRenderPlantCard.length);