...
// ============================================================
// APPROACH: Split HTML at<script> tags, translate HTML text only,
// leave JavaScript untouched, then reassemble.
// ============================================================

// Split into HTML parts and script parts
const scriptPattern = /<script\b[^>]*>[\s\S]*?<\/script>/gi;
const parts = html.split(scriptPattern);

// Each part is either HTML (translate) or JS (keep as-is)
// parts[0] = HTML before first script (translate)
// parts[1] = JS (keep)
// parts[2] = HTML between scripts (translate)
// etc.

function translateHtml(segment) {
  // Only translate text inside HTML elements
  // Use a map of specific translations for visible text
  const t = [
    // Page title
    ['Gardening Made Simple', '園藝變得更簡單'],
    // Meta description (keep English for SEO, or translate?)
    // Hero section
  ...
  for (const [eng, zh] of t) {
    segment = segment.split(eng).join(zh);
  }
  return segment;
}
...
// Fix lang attribute
result = result.replace(/<html lang="en">/, '<html lang="zh-Hant">');

// Fix nav: on Chinese page, show "EN" linking to index.html (not "中文" linking to index-zh.html)
result = result.replace(
  /<a href="index-zh\.html" class="text-sm font-medium text-forest-600 hover:text-forest-700 transition">[\u4e00-\u9fff\ufe0f\u20e3]+<\/a>/,
  '<a href="index.html" class="text-sm font-medium text-forest-600 hover:text-forest-700 transition">EN</a>'
);

// Fix browse-all-text: replace "Browse all X plants" with Chinese
// This is tricky because "plants" was being replaced. Let's fix it with a targeted replacement
result = result.replace(
  /<span id="browse-all-text">瀏覽全部 (\d+)種植物<\/span>/g,
  function(m, n) {
    return `<span id="browse-all-text">瀏覽全部 ${n} 種植物</span>`;
  }
);

fs.writeFileSync('plantglow/index-zh.html', result, 'utf8');
console.log('Done! File size:', fs.statSync('plantglow/index-zh.html').size);

// Verify function names preserved
const origFns = fs.readFileSync('plantglow/index.html', 'utf8').match(/function \w+/g);
const zhFns = result.match(/function \w+/g);
const missing = origFns ? origFns.filter(f => !zhFns.includes(f)) : [];
console.log('Missing JS functions:', missing.length === 0 ? 'None - all good!' : missing);
...
for (const s of checks) {
  console.log(result.includes(s) ? 'OK: ' + s : 'MISSING: ' + s);
}

// Check EN nav link
console.log(result.includes('href="index.html" class="text-sm font-medium text-forest-600 hover:text-forest-700 transition">EN</a>') ? 'OK: EN nav link' : 'MISSING: EN nav link');