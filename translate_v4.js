...
const edits = [
  ...
  // ---- Plant grid buttons (with icon on same line, then text on next line) ----
...
for (const [eng, zh] of edits) {
  if (!html.includes(eng)) {
    console.log('WARNING: Not found:', JSON.stringify(eng.substring(0, 60)));
    ok = false;
  }
  html = html.split(eng).join(zh);
}

if (ok) console.log('All replacements applied');

// Fix lang attribute
html = html.replace(/<html lang="en">/, '<html lang="zh-Hant">');

// Fix nav: on Chinese page, show "EN" linking to index.html
// (The button will say "EN" on the Chinese page to switch back to English)
html = html.replace(
...
const checks = ['園藝變得更簡單', '熱門植物', '全部植物', '我的最愛', 'showAllPlantsView', 'renderPlants', 'showLoginToast', 'openLoginRequiredModal', 'EN</a>'];
